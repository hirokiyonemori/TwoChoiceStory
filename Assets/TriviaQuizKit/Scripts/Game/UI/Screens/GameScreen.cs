...
namespace TriviaQuizKit
{
	...
	public class GameScreen : BaseScreen
	{
		...
		private void Start()
		{
			...
			var selectedQuestionType = (QuestionType)ES3.Load("question_type", 2);
		...
		private void LoadQuestions()
		{
			...
			var questionType = (QuestionType)ES3.Load("question_type", 2);
			switch (questionType)
			{
				case QuestionType.SingleChoice:
			...
			{
				var categoryIdx = ES3.Load("category", -1);
				if (categoryIdx >= 0)
				{
			...
			if (availableQuestions.Count > 0)
			{
				...
				switch (QuestionOrder)
				{
					case QuestionOrderType.Randomized:
						...
						break;
					// 指定された順番に読み込む
					case QuestionOrderType.Specified:
						idx = 0;
						break;
				}
		...
		private IEnumerator OpenGameFinishedPopupAsync()
		{
			...
			var questionType = ES3.Load("question_type", 2);
			var category = ES3.Load("category", -1);
